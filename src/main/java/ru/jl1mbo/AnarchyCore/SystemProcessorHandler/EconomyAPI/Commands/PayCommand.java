    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (!EconomyAPI.isRegister(nickname)) {
                player.sendMessage(EconomyAPI.PREFIX + "Игрок §6" + nickname + " §fне зарегистрирован§7!");
                return true;
            }
            if (!Utils.isDouble(args[0]) || Double.parseDouble(args[0]) < 0) {
                player.sendMessage(EconomyAPI.PREFIX + "Сумма может быть только положительным числом§7!");
                return true;
            }
            if (EconomyAPI.myMoney(player.getName()) < Double.parseDouble(args[0])) {
                player.sendMessage(EconomyAPI.PREFIX + "Вам не хватает §6монет §fдля перевода§7!\n§l§6• §rВаш баланс§7: §6" + String.format("%.1f", EconomyAPI.myMoney(player.getName())) + "");
                return true;
            }
            player.sendMessage(EconomyAPI.PREFIX + "Вы успешно перевели §6" + String.format("%.1f", Double.parseDouble(args[0])) + " §fигроку §6" + nickname);
