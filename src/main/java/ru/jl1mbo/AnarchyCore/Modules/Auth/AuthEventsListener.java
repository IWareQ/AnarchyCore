...
public class AuthEventsListener implements Listener {
	...
	private static void addFloatingKillsTops(Position pos, Player player) {
		...
		for (Map.Entry<String, Integer> entry : calculateScore(config).entrySet()) {
			stringBuilder.append("§l").append(entry.getKey()).append(" §7- §6").append(entry.getValue()).append(" §f⚔\n");
	...
	public void onPlayerQuit(PlayerQuitEvent event) {
		...
		if (playerTime.containsKey(player)) {
			MySQLUtils.query("UPDATE `Users` SET `GameTime` = '" + (AuthAPI.getGameTime(player.getName()) + (System.currentTimeMillis() / 1000L - playerTime.get(player))) + "' WHERE UPPER (`Name`) = '" + player.getName().toUpperCase() + "'");
		}
