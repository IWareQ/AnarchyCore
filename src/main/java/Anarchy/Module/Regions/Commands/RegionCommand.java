...
public class RegionCommand extends Command {
	...
	public boolean execute(CommandSender commandSender, String s, String[] args) {
		...
		case "help":
			new SimpleForm("Регионы", "Текст").addButton("Информация о Регионах").addButton("Мои Регионы").addButton("Список регионов, где Вас добавили").send(player, (target, form, data) -> {
				if (data == -1) return;
				if (data == 0) {
				...
				if (data == 1) {
					...
					new SimpleForm("Мои Регионы", "Ваши регионы: " + stringBuilder.toString()).send(player);
				}
				if (data == 2) {
					ArrayList<Integer> regionsData = SQLiteUtils.selectIntegerList("Regions.db", "SELECT `Region_ID` FROM `MEMBERS` WHERE `Username` = '" + playerName + "';");
					if (regionsData == null || regionsData.isEmpty()) {
					...
					new SimpleForm("Список регионов, где Вас добавили", "§fРегионы, в которых Вы состоите:" + stringBuilder.toString()).send(player);
				}
			});
		}
		return false;
