	...
	public boolean execute(CommandSender sender, String label, String[] args) {
		...
		switch (args[0]) {
			case "add": 
			{
				...
				if (addPlayer == null) {
					player.sendMessage(RegionsAPI.PREFIX + "§fИгрок §6" + args[1] + " §7- §3Оффлайн");
					return true;
				}
				if (player == addPlayer) {
				...
				if (SQLiteUtils.selectString("Regions.db", "SELECT `Username` FROM `MEMBERS` WHERE UPPER(`Username`) = \'" + playerName.toUpperCase() + "\' AND `Region_ID` = \'" + regionID + "\';") != null) {
					player.sendMessage(RegionsAPI.PREFIX + "§fИгрок §3" + addPlayer.getName() + " §fуже состоит в Вашем регионе§7!");
					return true;
				}
			...
			case "del": 
			{
				...
				if (!RegionsAPI.isRegionMember(delName, regionID)) {
					player.sendMessage(RegionsAPI.PREFIX + "§fИгрок §3" + delName + " §fне состоит в Вашем регионе§7!");
					return true;
				}
				Player delPlayer = Server.getInstance().getPlayerExact(delName);
				if (delPlayer != null) {
					delPlayer.sendMessage(RegionsAPI.PREFIX + "§fИгрок §3" + player.getName() + " §fудалил Вас из своего региона§7!");
				}
				player.sendMessage(RegionsAPI.PREFIX + "§fИгрок §3" + delName + " §fудален из региона§7! (" + StringUtils.getOnlineString(delName) + "§7)");
