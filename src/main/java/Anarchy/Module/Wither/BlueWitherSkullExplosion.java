			...
			for (int j = 0; j < this.rays; ++j) {
				for (int k = 0; k < this.rays; ++k) {
					if (i == 0 || i == mRays || j == 0 || j == mRays || k == 0 || k == mRays) {
						vector.setComponents((double)i / (double)mRays * 2.0 - 1, (double)j / (double)mRays * 2.0 - 1, (double)k / (double)mRays * 2.0 - 1);
						...
						for (double blastForce = this.size * (ThreadLocalRandom.current().nextInt(700, 1301)) / 1000.0; blastForce > 0; blastForce -= this.stepLen * 0.75) {
							int x = (int)pointerX;
							int y = (int)pointerY;
							int z = (int)pointerZ;
