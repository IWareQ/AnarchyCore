	...
	private Vector3 updateMove(int tickDiff) {
		if (!isImmobile()) {
			...
			if (this.followTarget != null && !this.followTarget.closed && this.followTarget.isAlive()) {
				...
				if (this.stayTime > 0 || this.distance(this.followTarget) <= (this.getWidth() + 0.0) / 2 + 0.05) {
					this.motionX = 0;
					this.motionZ = 0;
				} else {
			...
			if (this.target instanceof EntityCreature || before != this.target) {
				...
				double diff = Math.abs(x) + Math.abs(z);
				if (this.stayTime > 0 || this.distance(this.target) <= (this.getWidth() + 0.0) / 2 + 0.05) {
					this.motionX = 0;
					this.motionZ = 0;
				} else {
					this.motionX = this.getSpeed() * 0.15 * (x / diff);
			...
			if (this.isOnGround()) {
				this.motionY = RandomUtils.rand(0.15, 0.2);
			} else {
