	...
	public boolean execute(CommandSender sender, String label, String[] args) {
		if (sender instanceof Player) {
			if (args.length == 0) {
				...
				if (!dataFile.exists()) {
					player.sendMessage("§l§6• §r§fВ Хранилище купленных Предметов §6пусто §7:§fc§7!");
					return false;
				}
				...
				if (slotChange.getInventory() instanceof StorageChest) {
					...
					if (compoundTag != null && compoundTag.getString("UUID") != null) {
						...
						if (playerInventory.canAddItem(sourceItem)) {
							...
							playerInventory.addItem(sourceItem.clearCustomName().clearCustomBlockData().setLore().clearNamedTag());
