...
public class BonusCommand extends Command {
	...
	public boolean execute(CommandSender sender, String label, String[] args) {
		if (sender instanceof Player) {
			...
			Integer bonusID = SQLiteUtils.selectInteger("SELECT `Bonus` FROM `Users` WHERE UPPER(`Username`) = '" + player.getName().toUpperCase() + "';");
			if (bonusID == 0) {
				...
				SQLiteUtils.query("UPDATE `Users` SET `Bonus` = '1' WHERE UPPER(`Username`) = '" + player.getName().toUpperCase() + "';");
			} else if (bonusID == 1) {
				player.sendMessage("§l§6• §r§fБонус можно получить §61 §fраз за вайп§7!");
			}*/
			Map<String, String> test = SQLiteUtils.selectStringMap("SELECT * FROM `Users` WHERE UPPER(`Username`) = '" + player.getName().toUpperCase() + "';");
			player.sendMessage("TEST:" + test);
			player.sendMessage("TEST2:" + test.get("Permission"));
		}
		return false;
	}
}