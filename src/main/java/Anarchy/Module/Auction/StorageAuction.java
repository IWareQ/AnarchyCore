...
public class StorageAuction extends PluginBase {
	...
	public static Config getStorageAuctionConfig(Player player) {
	...
	public static void showStorageAuction(Player player, boolean firstTime) {
		...
		if (getStorageAuctionConfig(player).getAll().size() == 0) {
			player.sendMessage(AuctionAPI.PREFIX + "§fВ Вашем Хранилище Аукциона пусто§7!");
			return;
		}
		for (Map.Entry<String, Object> entry : getStorageAuctionConfig(player).getAll().entrySet()) {
