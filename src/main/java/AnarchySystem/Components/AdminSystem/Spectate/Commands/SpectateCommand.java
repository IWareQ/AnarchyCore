package AnarchySystem.Components.AdminSystem.Spectate.Commands;
...
public class SpectateCommand extends Command {
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length != 1) {
                player.sendMessage("§l§6• §r§fИспользование §7- §6/spectate §7(§6игрок§7)");
                return true;
            }
            Player target = Server.getInstance().getPlayer(args[0]);
            if (target == null) {
                player.sendMessage(SpectateAPI.PREFIX + "§fИгрок §6" + args[0] + " §fне в сети§7!");
                return true;
            }
