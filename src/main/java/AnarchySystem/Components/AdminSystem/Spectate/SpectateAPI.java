package AnarchySystem.Components.AdminSystem.Spectate;
...
public class SpectateAPI {
    ...
    public static void addSpectate(Player player, Player target) {
        ...
        if (isSpectate(player.getName())) {
            ...
            player.getInventory().setItem(4, Item.get(Item.CHEST).setCustomName("§r§fПросмотреть Инвентарь").setLore("§r§l§6• §r§fИспользуйте чтобы просмотреть §6Инвентарь §fнаблюдаемого§7!"));
    ...
    public static void removeSpectate(Player player) {
        ...
        if (isSpectate(player.getName())) {
            ...
            player.teleport(new Position(config.getInt(player.getName().toLowerCase() + ".Position.X") + 0.5, config.getInt(player.getName().toLowerCase() + ".Position.Y"), config.getInt(player.getName().toLowerCase() + ".Position.Z") + 0.5, Server.getInstance().getLevelByName(config.getString(player.getName().toLowerCase() + ".Position.World"))));
